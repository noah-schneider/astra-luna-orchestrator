...
The worker is the native Codex model `gpt-6-sol` at `high`. There is no
...
The installer creates `$CODEX_HOME/agents/astra_sol_builder.toml` with its own
...
- The child thread/session metadata shows `gpt-6-sol` at `high`.

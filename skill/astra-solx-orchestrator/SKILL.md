---
name: astra-solx-orchestrator
description: Plan and execute substantial multi-file builds, features, migrations, and refactors with Astra as orchestrator and GPT-6 Sol at xhigh as the native Codex implementation worker. Use when the user requests Solx or Sol xhigh for phased planning and delegated build execution, including existing Superpowers or GSD plans. Skip trivial edits and explicitly single-agent tasks. Worker children must not invoke this orchestration skill.
---
...
runtime delegation is verified from these instructions or a worker's self-report.
...
## 2. Confirm native delegation before delegating

Read `references/routing.md`. Run the read-only `scripts/doctor.py` with the
...
`astra_solx_builder` role is available. Do not change the root model or effort.
...
than repeating a paid test per task. For a first delegated task, use a small
real, useful implementation bundle and inspect host/session metadata afterward.
No fake model-name check and no silent fallback. Do not run `subagents certify`,
`test-model --live`, a smoke test or another paid probe as package setup. If native
delegation cannot be established,
...
the installed `astra_solx_builder` role. Do not invent a slash command or tool

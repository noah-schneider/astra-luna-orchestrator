...
def validate(plan: object, root: Path) -> dict:
    ...
    capacity = plan.get("max_sol_workers", 1)
    if type(capacity) is not int or capacity not in {1, 2}:
        raise PlanError("max_sol_workers must be 1 or 2.")
    ...
    for task in tasks.values():
        ...
        if task.get("executor") not in {"astra", "sol"}:
            raise PlanError(f"Task {name} executor must be astra or sol.")
    ...
    for group, members in groups.items():
        if len(members) > capacity or any(t["executor"] != "sol" for t in members):
            raise PlanError(f"Parallel group {group} exceeds worker capacity or includes a non-Sol executor.")
    ...
    return {"status": "structure-valid", "phases": len(phases), "tasks": len(tasks), "max_sol_workers": capacity,

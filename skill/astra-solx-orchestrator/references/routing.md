...
The worker is the native Codex model `gpt-6-sol` at `xhigh`. There is no
...
The installer creates `$CODEX_HOME/agents/astra_solx_builder.toml` with its own
...
- The child thread/session metadata shows `gpt-6-sol` at `xhigh`.
...
unverified. A response saying "I am GPT-6 Sol" is not evidence. Do not run a
